开发须知：
开发任务分为三点，如下：
1.对接支付，在用户下单后，通知本系统，然后对其账户进行更新，然后需要对提现系统进行安全性测试
2.对接分享，对商超后台的商品进行对接，生成推广链接和二维码，另外一种模式是商品推广出去若干次后可免费购买该商品
3.代理分成修改，将系统的代理分成模式修改成所有上级均分利润

项目可直接启动，数据库用的阿里云服务器上的数据库，有两个数据库。
